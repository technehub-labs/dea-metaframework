...
| "persists" (substrate as enabler) | → Enablement & Operations | Persistence requires a mechanism: the engine and the means that make execution possible. |
...
| 6 | Enablement & Operations | The domain that establishes, sustains, and governs the means and mechanisms through which the enterprise executes, delivers, and maintains its capabilities and value exchanges; operational processes, enabling technology, physical and virtual infrastructure, service mechanisms, and operational controls. |
...
- **Domain.** One of the seven canonical enterprise concern dimensions of
  ...
  Direction; Agency & Organization; Party & Relationship; Product & Value;
  Enablement & Operations; Finance & Accounting.
...
The monetization spine: **Perform → Account → Bill** (Enablement & Operations × Operate → Finance × Operate → Finance × Measure) - exists in every enterprise. What changes is the payload.
...
| **Enablement & Operations** | Traffic forecast | Network design | Circuit prov. | Site cut-over | NOC operations (24/7) | Fault mgm (TT) | Site decom. |
...
- The **Enablement & Operations × Activate** cell, network attach via HLR/HSS, is the telco's highest-risk handoff; the matrix isolates it for investment.
...
| Layer 3 — Digital Ecosystem & Intelligence | Party & Relationship, Product & Value, Enablement & Operations | Build → Operate |
| Layer 4 — Technology & Execution | Enablement & Operations | Build → Operate |
...
## 21. Position in the TechNeHub Labs Ecosystem

### 21.1 Ecosystem reference

TechNeHub Labs is an open-source, vendor-neutral reference ecosystem for Digital Enterprise Architecture (DEA). The ecosystem structure evolves continuously, so this report does not embed a repository snapshot. For the current ecosystem structure, see the TechNeHub Labs organization ([github.com/technehub-labs](https://github.com/technehub-labs)) and the ecosystem portal ([technehub-labs.github.io](https://technehub-labs.github.io)).

The centerpiece adoption programme is **DERA**, the Digital Enterprise Reference Architecture, which assembles the catalogs into a four-phase adoption programme: Discover & Define → Design & Build → Deploy & Operate → Evolve & Retire.

### 21.2 The Core Relationship

**WSF grounds; OpenDEA specializes; ECF organizes; the metamodel represents; catalogs instantiate.**

ECF says *how any enterprise is organized*: 7 domains × 7 stages, derived from a single axiom, giving 49 classification coordinates. The DEA metamodel says *how enterprise concepts are formally represented*. The catalogs say *which governed concepts exist*. ECF coordinates contextualize metamodel entities and catalog entries; they do not contain them, and no catalog is generated by enumerating coordinates. The normative statement of this position is [`framework/architecture.md`](./framework/architecture.md) (CR-ECF-001).

### 21.3 Domain → Catalog Mapping

The following mapping is an illustrative snapshot. The TechNeHub Labs portfolio is authoritative for the current repository structure.
...
| Agency & Organization | `dea-catalog-digital-business-service-factory` | HR business service — workforce capability maps and SLA governance. |
...
| Enablement & Operations | `dea-catalog-patterns`, `dea-catalog-solution-hub` | Architecture patterns (CQRS, Saga, Strangler Fig) + workflow accelerators + technology and physical enablers. |
...
### 21.5 Construct → Metamodel Entity Mapping

The following mapping is an illustrative snapshot from the original framework derivation. The canonical entity set and its ECF profile binding live in `dea-metamodel`.
...
### 21.6 Where ECF Fits Best

ECF is consumed as an OpenDEA profile by `dea-metamodel` and as classification context by the reference catalogs. The Business Capability catalog treats ECF coordinates as classification context rather than capability identity. The Business Process catalog treats an ECF intersection as Process Context rather than as a process. ECF's derived artifacts (entity catalog, capability map, traceability matrix) are the conceptual schema; the catalog repositories provide the populated, governed instances.

### 21.7 Gaps ECF Reveals in the DEA Ecosystem

| Gap | What ECF surfaces |
|-----|-------------------|
| Supply has no dedicated catalog post-rename (was `dea-catalog-reference-models`/solution-hub) | Infrastructure patterns are scattered; the absorption of the prior "Supply & Resources" content into Enablement & Operations argues for a consolidated enablement catalog. |
...
The Enterprise Concept Framework is a lens. Point it at any enterprise, for-profit or not, in any industry, and the same seven domains and seven stages come into focus. At TechNeHub Labs, it is the enterprise organizing framework within OpenDEA: the coordinate system that contextualizes what the metamodel formally represents and the catalogs instantiate.
